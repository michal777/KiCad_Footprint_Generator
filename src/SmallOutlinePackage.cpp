...
SmallOutlinePackage::SmallOutlinePackage() {
	x_pads_center = 0;
	x_pads_outer = 0;
	pads_count_x = 0;
	pads_count_y = 0;
...
void SmallOutlinePackage::ReadParameters(char table[100][100][500], int row)
{
	int column;
...
void SmallOutlinePackage::CalcAssemblyScale(void)
{
...
void SmallOutlinePackage::CalcLandPattern(void)
{
	double F;
	double P;
	double j_t;
...
void SmallOutlinePackage::PrintFootprint(FILE *footprint_file)
{
...
void SmallOutlinePackage::GeneratePads(FILE *footprint_file)
{
...
void SmallOutlinePackage::GenerateAssembly(FILE *footprint_file, double assem_scale)
{
	double start_line_x;
	double end_line_x;
	double start_line_y;
...
void SmallOutlinePackage::GenerateCourtyard(FILE *footprint_file)
{
	double start_line_x;
	double end_line_x;
	double start_line_y;
	...
	na_x = na1_x = x_pads_center / 2 + pad_length / 2 + CRTYD_CLEARANCE;
	na1_y = nb1_y = r05up (pitch * (pads_count_y - 1) / 2 + pad_width / 2 + MAX (2.5 * SILKS_LINE_WIDTH, CRTYD_CLEARANCE));
	na_y = nb_y = pitch * (pads_count_y - 1) / 2 + pad_width / 2 + CRTYD_CLEARANCE;
	nb_x = nb1_x = nc_x = r05up (body_size_max_x / 2 + 1.5 * SILKS_LINE_WIDTH);
	nc_y = r05up (body_size_max_y / 2 + 1.5 * SILKS_LINE_WIDTH);
...
void SmallOutlinePackage::GenerateSilkScreen(FILE *footprint_file)
{
	double start_line_x;
	double end_line_x;
	double start_line_y;

...
PinHeadersAngled::PinHeadersAngled() {
	pads_rows_spacing = 0;
	pads_per_row = 0;
	rows_count = 0;
	pitch = 0;
...
void PinHeadersAngled::ReadParameters(char table[100][100][500], int row) {
	int column;
	for (column = 0; '\0' != table[0][column][0]; ++column)
	{
		...
		if (!strcmp(table[0][column], "Body height"))
			body_height = atof (table[row][column]);
		...
		if (!strcmp(table[0][column], "Pin length max"))
			pin_length_max = atof (table[row][column]);
		if (!strcmp(table[0][column], "Overall length"))
			overall_length = atof (table[row][column]);
	}
}

void PinHeadersAngled::CalcAssemblyScale(void) {
...
void PinHeadersAngled::GeneratePads(FILE* footprint_file) {
...
void PinHeadersAngled::PrintFootprint(FILE* footprint_file) {
...
void PinHeadersAngled::GenerateAssembly(FILE* footprint_file, double assem_scale) {
	double start_line_x;
	double end_line_x;
	double start_line_y;
	double end_line_y;
...
void PinHeadersAngled::GenerateCourtyard(FILE* footprint_file) {
	double start_line_x;
	double end_line_x;
	double start_line_y;
	double end_line_y;
...
void PinHeadersAngled::GenerateSilkScreen(FILE* footprint_file) {
	double start_line_x;
	double end_line_x;
	double start_line_y;
	double end_line_y;
	...
	double na_x = overall_length - pin_thickness / 2 - pitch * (rows_count - 1) / 2;
	double na_y;
	double nb_y;
	double nc_x = overall_length - pin_thickness / 2 - pin_length_max - pitch * (rows_count - 1) / 2;
	double nd_x = nc_x;
	double nd_y;
	double ne_y;
	double nf_x = overall_length - pin_thickness / 2 - pin_length_max - body_height - pitch * (rows_count - 1) / 2;;
	double ng_x = nf_x;
	double ng_y;
	double nh_y;
	double ni_x = pad_length / 2 - pitch * (rows_count - 1) / 2 + 1.5 * SILKS_LINE_WIDTH;

	double np_x = body_width_max / 2 + CRTYD_CLEARANCE_2 - 2 * SILKS_LINE_WIDTH;
	double np_y = body_length_max / 2 + CRTYD_CLEARANCE_2 - 2 * SILKS_LINE_WIDTH;
	double nq_x = np_x - pitch;
	double nq_y = np_y - pitch;
	...
	{
		start_line_y = end_line_y = -np_y;
		start_line_x = -np_x;
		end_line_x = -nq_x;
		fprintf(footprint_file, "  (fp_line (start %s %s) (end %s %s) (layer F.SilkS) (width %s))\n", f2c(start_line_x + tht_offs_x), f2c(start_line_y + tht_offs_y), f2c(end_line_x + tht_offs_x), f2c(end_line_y + tht_offs_y), f2c(SILKS_LINE_WIDTH));
		start_line_x = end_line_x = -np_x;
		start_line_y = -np_y;
		end_line_y = -ne_y;
		fprintf(footprint_file, "  (fp_line (start %s %s) (end %s %s) (layer F.SilkS) (width %s))\n", f2c(start_line_x + tht_offs_x), f2c(start_line_y + tht_offs_y), f2c(end_line_x + tht_offs_x), f2c(end_line_y + tht_offs_y), f2c(SILKS_LINE_WIDTH));
	}
...
void PinHeadersAngled::CalcOffset(void) {

...
QuadFlatPackage::QuadFlatPackage() {
	x_pads_center = 0;
	y_pads_center = 0;
	x_pads_outer = 0;
	y_pads_outer = 0;
...
void QuadFlatPackage::ReadParameters(char table[100][100][500], int row)
{
	int column;
...
void QuadFlatPackage::CalcAssemblyScale(void)
{
...
void QuadFlatPackage::CalcLandPattern(void)
{
	double F;
	double P;
	double j_t_x;
...
void QuadFlatPackage::PrintFootprint(FILE *footprint_file)
{
	...
	fprintf(footprint_file, "  (fp_text value %s (at 0 %s) (layer F.SilkS) hide\n", footprint_name, f2c(y_pads_center / 2 + pad_length_x / 2 + SILKS_TEXT_SIZE));
	fprintf(footprint_file, "    (effects (font (size %s %s) (thickness %s)))\n  )\n", f2c(SILKS_TEXT_SIZE), f2c(SILKS_TEXT_SIZE), f2c(SILKS_LINE_WIDTH));
...
void QuadFlatPackage::GeneratePads(FILE *footprint_file)
{
...
void QuadFlatPackage::GenerateAssembly(FILE *footprint_file, double assem_scale)
{
	double start_line_x;
	double end_line_x;
	double start_line_y;
...
void QuadFlatPackage::GenerateCourtyard(FILE *footprint_file)
{
	double start_line_x;
	double end_line_x;
	double start_line_y;
	...
	na_x = na1_x = x_pads_center / 2 + pad_length_x / 2 + CRTYD_CLEARANCE;
	na1_y = nb1_y = r05up (pitch_y * (pads_count_y - 1) / 2 + pad_width_x / 2 + MAX (2.5 * SILKS_LINE_WIDTH, CRTYD_CLEARANCE));
	na_y = nb_y = pitch_y * (pads_count_y - 1) / 2 + pad_width_x / 2 + CRTYD_CLEARANCE;
	nb_x = nb1_x = nc_x = r05up (body_size_max_x / 2 + 1.5 * SILKS_LINE_WIDTH);
...
void QuadFlatPackage::GenerateSilkScreen(FILE *footprint_file)
{
	double start_line_x;
	double end_line_x;
	double start_line_y;
	...
	double nb_y = pitch_y * (pads_count_y - 1) / 2 + pad_width_x / 2 + 1.5 * SILKS_LINE_WIDTH;
	double nc_x = pitch_x * (pads_count_x - 1) / 2 + pad_width_y / 2 + 1.5 * SILKS_LINE_WIDTH;
	double nd_x = x_pads_center / 2 + pad_length_x / 2;
